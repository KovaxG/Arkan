...
udeltax = m.Udeltax(:, u); % Select the appropriate change in states
xplus = sat(x+udeltax, m.minx, m.maxx); % Fit next state between min- and max- x

if all(xplus == m.x_goal),
    % reached goal
    rplus = m.rew_goal; % reached goal reward
    terminal = 1;
elseif ~isempty(findflat(xplus, m.x_obst)),
    % hit an obstacle, reset to earlier position
    xplus = x;
    rplus = m.rew_obst; % Incur a penalty
    terminal = 0;
else
    % regular move
    rplus = m.rew_step; % normal reward
    terminal = 0;
end;    
