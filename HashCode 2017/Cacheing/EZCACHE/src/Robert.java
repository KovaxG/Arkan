...
public class Robert {
	...
	public void sort() {
		...
		for (Cache cache : caches) {
			...
			for (EndPoint e : endpoints) {
				...
				// im not sure if this is actually faster or slower(using a temp
				// var)
			...
			/// construct a list with the videos and afferent scores from the
			/// endpoints selected
			...
			// TODO write code for replaceing already insered videos, for a
			// better combination
			for (videosWithScore vScore : videosWithScores) {
				if (vScore.video.getSize() < cache.getSize() && !cache.getVideos().contains(vScore.video)) {
					...
					/// remove all requests that contain videos that have been
	...
	public int calculateScore() {
		try {
			return calculateScore(inputFile, caches);
	...
	public int calculateScore(String inputFile, ArrayList<Cache> filledCaches) throws FileNotFoundException {
		try {
			...
			int videoCount = sc.nextInt();
			int endpointCount = sc.nextInt();
			int requestDesctriptionCount = sc.nextInt();
			/*int cacheCount = */sc.nextInt();
			...
			for (int i = 0; i < endpointCount; i++) {
				...
				for (int j = 0; j < m; j++) {
					int toCacheNr = sc.nextInt();
					int toCacheLatency = sc.nextInt();
					ep.getChacheList().add(new Pair<Cache, Integer>(filledCaches.get(toCacheNr), toCacheLatency));
				}
				endpoints.add(ep);
			}
