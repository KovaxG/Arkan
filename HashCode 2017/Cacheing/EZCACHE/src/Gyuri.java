...
public class Gyuri {
	public static void main(String args[]) {
		...
		System.err.println("Starting " + file[i]);
		
		long t0 = System.currentTimeMillis();
			
		Robert.inputFile = "inputs/" + file[i] + ".in";
		Robert.outputFile = "outputs/" + file[i] + ".txt";
		...
		for (Request r : allRequests) {
			...
			for (Pair<Cache, Integer> pair : endP.getChacheList()) {
				...
				if (c.getSize() >= r.getVideo().getSize() && 
					!c.getVideos().contains(r.getVideo()) &&
					lag < minLag) {
			...
			for (Request req : ep.getRequestList()) {
				for (Pair<Cache, Integer> p : ep.getChacheList()) {
					...
					if (c.getVideos().contains(req.getVideo())) {
						//System.out.println("" + (ep.getDataCenterLatency() - lag) + " " + req.getDemand());
...
class EndPoint {
	...
	public void setRequestList(ArrayList<Request> requestList) {this.requestList = requestList;}
	public ArrayList<Pair<Cache, Integer>> getChacheList() {return chacheList;}
	public void setChacheList(ArrayList<Pair<Cache, Integer>> chacheList) {this.chacheList = chacheList;}
	public int getDataCenterLatency() {return dataCenterLatency;}
	public void setDataCenterLatency(int dataCenterLatency) {this.dataCenterLatency = dataCenterLatency;}
} // End of EndPoint
